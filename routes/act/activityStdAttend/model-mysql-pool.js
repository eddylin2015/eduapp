...
function list(userId, limit, token, cb) {
    ...
    pool.getConnection(function (err, connection) {
        // Use the connection
        connection.query(
            'SELECT * FROM `ActStdAttend` order by id DESC LIMIT ? OFFSET ?', [limit, token],
            (err, results) => {
                if (err) {
                    cb(err);
                    return;
...
function listBy(userId, limit, token, cb) {
    ...
    pool.getConnection(function (err, connection) {
        connection.query(
            'SELECT * FROM `ActStdAttend` WHERE `createdById` = ? LIMIT ? OFFSET ?',
            [userId, limit, token],
            (err, results) => {
                if (err) {
                    cb(err);
...
function listTimestampBy(userId, activity, sdate,edate, limit, token, cb) {
    ...
    pool.getConnection(function (err, connection) {
        connection.query(
            'SELECT * FROM `ActStdAttend` WHERE `activity` = ?  and (`logDate` between ? and ? ) order by  id  LIMIT ? OFFSET ?',
...
function create(userId, data, cb) {
    console.log(data);
    pool.getConnection(function (err, connection) {
        connection.query('INSERT INTO `ActStdAttend` SET ?', data, (err, res) => {
            if (err) {
                cb(err);
                return;
            }
...
function read(userId, id, cb) {
    ...
    pool.getConnection(function (err, connection) {
        connection.query(
            'SELECT * FROM `ActStdAttend` WHERE `id` = ? ', id, (err, results) => {
...
function update(userId, id, data, cb) {
    pool.getConnection(function (err, connection) {
        connection.query(
            'UPDATE `ActStdAttend` SET ? WHERE `id` = ?  and `createdById` = ?', [data, id, userId], (err) => {
                if (err) {
                    cb(err);
                    return;
                }
...
function _delete(userId,id, cb) {
    pool.getConnection(function (err, connection) {
        connection.query('DELETE FROM `ActStdAttend` WHERE `id` = ?  and  `createdById` = ?',[ id, userId ],  cb);
        connection.release();
    });
}
...
function createSchema(config) {
    ...
    USE \`bookshelf\`;
    CREATE TABLE IF NOT EXISTS \`act\`.\`ActStdAttend\` (
